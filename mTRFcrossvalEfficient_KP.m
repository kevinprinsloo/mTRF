function [r,p,mse,pred,model] = mTRFcrossvalEfficient_KP(stim,resp,fs,map,tmin,tmax,lambda)
...
for i = 1:numel(x)
    ...
    if (size(x{i},1)*(size(x{i},2)+size(x{i},2)*length(tmin:tmax)))*8 < physical_memory_available_bytes
        ...
        X{1} = XGenEfficient_KP(x{i},tmin:tmax); % KP added < XGenEfficient_KP > function for ABR
    else
...
for i = 1:trials_number
    ...
    if (size(x{i},1)*(size(x{i},2)+size(x{i},2)*length(tmin:tmax)))*8 < physical_memory_available_bytes
        %     X{1} = [ones(size(x{i})),lagGen(x{i},tmin:tmax)];
        X{1} = XGenEfficient_KP(x{i},tmin:tmax);
    else
        error('Insufficient RAM available, unable to proceed.')
    end
