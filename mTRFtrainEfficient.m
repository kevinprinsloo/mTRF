function [model,t,c] = mTRFtrainEfficient(stim,resp,fs,map,tmin,tmax,lambda)
...
if (size(x,1)*(size(x,2)+size(x,2)*length(tmin:tmax)))*8 < physical_memory_available_bytes
%     tic; X = [ones(size(x)),lagGen(x,tmin:tmax)]; toc

%     X = zeros(size(x,1),size(x,2)+size(x,2)*length(tmin:tmax));
%     X(:,1:size(x,2)) = ones(size(x));
%     X(:,size(x,2)+1:end) = lagGen(x,tmin:tmax);

    X = XGenEfficient(x,tmin:tmax);
else
